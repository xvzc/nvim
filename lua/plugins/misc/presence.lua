...
setmetatable(file_assets, meta)

require("presence").setup({
	-- General options
	auto_update = true, -- Update activity based on autocmd events (if `false`, map or manually execute `:lua package.loaded.presence:update()`)
	neovim_image_text = "A perfect toy itself", -- Text displayed when hovered over the Neovim image
	...
	buttons = {
		{
			...
			url = "https://github.com/xvzc/nvim",
		},
	...
	file_assets = file_assets, -- Custom file asset definitions keyed by file names and extensions (see default config at `lua/presence/file_assets.lua` for reference)
